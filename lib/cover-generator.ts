...
export async function maybeGenerateAndSaveCover({
  ...
  force = false,
  feedback = null,
}: ForceGenerateParams): Promise<{ coverUrl?: string; skipped: boolean; skipReason?: string }> {
  const log = (msg: string) => console.log(`[cover-gen ${bookId}] ${msg}`);

  const OPENAI_API_KEY = process.env.OPENAI_API_KEY;
  if (!OPENAI_API_KEY) {
    log("SKIPPED: OPENAI_API_KEY env var is not set");
    return { skipped: true, skipReason: "OPENAI_API_KEY not set" };
  }

  if (!title.trim()) {
    log("SKIPPED: title is empty");
    return { skipped: true, skipReason: "empty title" };
  }
  ...
  if (
    ...
    log(`SKIPPED: author is missing or placeholder ("${author}")`);
    return { skipped: true, skipReason: `invalid author: "${author}"` };
  }

  if (!force && existingCoverUrl && isGeneratedCoverUrl(existingCoverUrl)) {
    log("SKIPPED: already has a generated cover (use force=true to regenerate)");
    return { skipped: true, skipReason: "already has generated cover" };
  }

  log(`Starting generation: "${title}" by "${authorTrimmed}" (force=${force})`);
  ...
  log(`Using image model: ${imageModel}, reference cover: ${referenceCoverUrl ? "yes" : "no"}, style hint: ${coverStyleHint ? "yes" : "no"}`);
  ...
  try {
    ...
    log("OpenAI image generation succeeded");
  } catch (openaiError) {
    const errorMessage = openaiError instanceof Error ? openaiError.message : String(openaiError);
    log(`OpenAI image generation FAILED: ${errorMessage}`);
    throw new Error(`OpenAI image generation failed: ${errorMessage}`);
  }

  const b64 = img.data?.[0]?.b64_json;
  if (!b64) {
    log("OpenAI returned no b64_json data — model may not support base64 output");
  ...
  if (updateError) {
    log(`DB update FAILED: ${JSON.stringify(updateError)}`);
    throw new Error("Failed to update cover_url.");
  }

  log(`SUCCESS: cover saved to ${publicUrl}`);
